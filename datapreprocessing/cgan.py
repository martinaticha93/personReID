...
def cgan() -> Tuple[tf.Tensor, tf.Tensor, tf.Operation, tf.Operation, tf.Tensor]:
    """
    create a tensorflow model with conditional GAN, and returns tensors and placeholders
    useful in manipulation with the model:
        input_batch: tf.Tensor of shape [None, 256,256,3], for edges images
        target_batch, tf.Tensor of shape [None, 256,256,3], for original images
        gen_train: when run, perform one step of Adam optimizer on generator model
        discrim_train: when run, perform one step of Adam optimizer on discriminator model
        outputs: outputs of the forward pass through generator
    """
    ...
    def batchnorm(inputs):
        return tf.layers.batch_normalization(inputs, axis = 3, epsilon = 1e-5,
            momentum = 0.1, training = True, gamma_initializer = tf.random_normal_initializer(1.0, 0.02))    
    ...
    def gen_conv(batch_input, out_channels, separable_conv = False):
        # Convolutional layer at generator. Takes a tensor of shape [batch, in_height, in_width, in_channels]
        #and outputs a tensor of shape [batch, out_height, out_width, out_channels]
        initializer = tf.random_normal_initializer(0, 0.02)
        if separable_conv:
            return tf.layers.separable_conv2d(batch_input, out_channels, kernel_size = 4,
                strides = (2, 2), padding = "same", depthwise_initializer = initializer,
    ...
    def gen_deconv(batch_input, out_channels, separable_conv=False):
        # Deonvolutional layer at generator. Takes a tensor of shape 
        #[batch, in_height, in_width, in_channels] and outputs a tensor of shape
        ...
        if separable_conv:
            ...
            return tf.layers.separable_conv2d(resized_input, out_channels,
                    kernel_size = 4, strides = (1, 1), padding = "same",
    ...
    return input_batch, target_batch, gen_train, discrim_train, outputs